...
def fetch_wallets(cursor):
    wallets = cursor.execute(
        "SELECT address FROM public.wallets ORDER BY rank ASC").fetchall()
    return wallets
...
def generate_balance_timeseries(wallets, connection, cursor):
    for wallet in tqdm(wallets, desc = 'Wallet', position = 0):
        ...
        firsttxtime = txs[0][0]
        candlesbefore = cursor.execute("SELECT time FROM public.klines WHERE time < %s ORDER BY time ASC", (firsttxtime,)).fetchall()
        ...
        for itrtr in tqdm(range(len(txs)-1), desc= 'Candles between', position = 1, leave = False):
            ...
            candles = cursor.execute("SELECT time FROM public.klines WHERE (time < %s AND time >= %s) ORDER BY time ASC", (tx2time, tx1time)).fetchall()
        ...
        candlesafter = cursor.execute("SELECT time FROM public.klines WHERE time >= %s ORDER BY time ASC", (lasttxtime,)).fetchall()
